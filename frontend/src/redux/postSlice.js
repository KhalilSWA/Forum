...
export const updatePost = createAsyncThunk('post/updatePost', async (data, { rejectWithValue }) => {
  try {
    ...
    const res = await axios.put(`posts/${data.post.id}`,{
       title: data.data.title,
       content: data.data.content,
       price: data.data.price
      }, 
...
export const deletePost = createAsyncThunk('posts/deletePost', async (post_id, { rejectWithValue }) => {
  ...
  extraReducers(builder) {
    builder
      ...
      .addCase(deletePost.fulfilled, (state, { payload }) => {
        state.auth = true
        state.loading = false   
        state.posts = state.posts.filter(post => post.post_id !== payload.post_id)
      })
