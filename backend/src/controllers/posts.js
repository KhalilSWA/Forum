...
async function createPost(req, res) {
    ...
    try {
        ...
        await post.save()
        res.status(200).send({msg:"added post",post});
    } catch(error) {
...
async function myPosts (req,res) {
    try{
        let posts=await Post.find({user_id: ObjectId(req.user.id)});
...
async function getPost(req, res) {
    try {
        ...
        let post = await Post.findOne({_id: ObjectId(post_id)}).populate({path: 'user_id', select: 'userName ', model: User});
